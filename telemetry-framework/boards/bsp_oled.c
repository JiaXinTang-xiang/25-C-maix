 ...
 *             0.96寸OLED使用SSD1306驱动, 本文件包括OLED初始化函数, 以及OLED设置函数, GRAM操作函数,
 *             oled显示数字、字符、字符串函数, 以及显示RoboMaster LOGO函数
 * @note
 * @history
...
/**
  ...
  */
/**
  * @brief          写数据或指令到OLED, 如果使用SPI请重写此函数
  * @param[in]      dat: 要写入的数据字节
  * @param[in]      cmd: OLED_CMD 表示写入指令字节; OLED_DATA 表示写入数据字节
  * @retval         none
  */
...
/**
  ...
  */
/**
  * @brief          操作GRAM内存(128*8char大小的二维数组)
  * @param[in]      pen: 操作类型.
                    PEN_CLEAR: 清零为0x00
                    PEN_WRITE: 置一为0xff
                    PEN_INVERSION: 按位取反
...
/**
  * @brief          设置光标位置(x,y)
  * @param[in]      x:x坐标, 范围 0 到 127
  * @param[in]      y:y坐标, 范围 0 到 7
  * @retval         none
  */
...
/**
  ...
  */
/**
  * @brief          画GRAM中的一个位,相当于在屏幕上画一个点
  * @param[in]      x:x坐标,  [0,X_WIDTH-1]
  * @param[in]      y:y坐标,  [0,Y_WIDTH-1]
  * @param[in]      pen: 操作类型,
                        PEN_CLEAR: 清除 (x,y) 为 0
                        PEN_WRITE: 绘制 (x,y) 为 1
                        PEN_INVERSION: (x,y) 值取反
  * @retval         none
  */
...
/**
  * @brief          显示一个字符串
  * @param[in]      row: 字符串的起始行
  * @param[in]      col: 字符串的起始列
  * @param[in]      chr: 字符串指针
  * @retval         none
  */
...
/**
  * @brief          格式化打印
  * @param[in]      row: 起始行,0 <= row <= 4;
  * @param[in]      col: 起始列, 0 <= col <= 列最大值 20;
  * @param[in]      *fmt:格式化字符串指针
  * @note           如果字符串长度超过一行，多余的字符会被截断
  * @retval         none
  */
